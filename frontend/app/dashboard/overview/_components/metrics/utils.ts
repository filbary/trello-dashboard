...
export const getUsersCardCount = (
  user: User,
  listsWithCards: ListWithCards[]
): number => {
  return listsWithCards.reduce((totalCount, list) => {
    const userCardsCount = list.cards.filter((card) =>
      card.idMembers.includes(user.id)
    ).length;
...
export const countActionsInLastDays = (
  actions: Action[],
  days: number
): number => {
  const now = Date.now();
...
const getAverageTaskCompletionTime = (
  createActions: Action[],
  updateActions: Action[]
): string => {
  return parseHoursToString(
    getSumOfMeanTimes(['To Do', 'In Progress'], createActions, updateActions) ||
      0
  );
};
...
const getPendingTasks = (listsWithCards: ListWithCards[]): number => {
  ...
  return listsWithCards.flatMap((list) =>
    list.cards.filter(
      (card) =>
        card.due && new Date(card.due).getTime() < now && !card.dueComplete
    )
  ).length;
};
...
const getRecentTasksDone = (
  updateActions: Action[]
): { cardName: string; completedBy: string; completedOn: string }[] => {
...
export const prepareAllMetrics = (
  listsWithCards: ListWithCards[],
  updateActions: Action[],
  createActions: Action[]
): Metrics => {
  return {
    ...
    averageTaskCompletionTime: getAverageTaskCompletionTime(
      createActions,
      updateActions
    ),
...
export function preparePieChartData(listsWithCards: ListWithCards[]) {
  ...
  return listsWithCards.map((list) => ({
    ...
    fill: `hsl(var(--chart-${
      Object.keys(nameToConfigKey).indexOf(list.name) + 1
    }))`
  }));
}
...
export function prepareAreaGraphData(
  ...
  const dateCounts: Record<
    string,
    { frontend: number; backend: number; devops: number }
  > = {};
